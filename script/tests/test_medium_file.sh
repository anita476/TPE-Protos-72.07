...
# CONFIGURATION
FILE_SIZE_MB=25
MEDIUMFILE="/tmp/mediumfile.bin"
HTTP_PORT=8082
PROXY=localhost:1080
TMP_DIR="/tmp/socks5_medium_test"
DOWNLOADED_FILE="$TMP_DIR/mediumfile_downloaded.bin"

# Create a large file with random data if it doesn't exist
if [ ! -f "$BIGFILE" ]; then
    echo "Creating $FILE_SIZE_MB MB file at $MEDIUMFILE..."
    dd if=/dev/urandom of="$MEDIUMFILE" bs=1M count="$FILE_SIZE_MB" status=progress
fi
...
curl -v --socks5 $PROXY "http://localhost:$HTTP_PORT/$(basename "$MEDIUMFILE")" -o "$DOWNLOADED_FILE" \
...
ORIG_HASH=$(sha256sum "$MEDIUMFILE" | awk '{print $1}')
