...
static int get_username(char *username, int size) {
    char *input = ui_get_input("Login", "Enter username:", 0);
    if (!input) {
        return -1;
    }
...
static int get_password(char *password, int size) {
    char *input = ui_get_input("Login", "Enter password:", 1);
    if (!input) {
        return -1;
    }
...
static void change_buffer_size() {
    ...
    char *input = ui_get_input("Buffer Size", "Enter new buffer size (KB):", 0);
    if (!input) {
        return;
    }
    ...
    if (!ui_get_confirmation("Confirm Change", confirm_msg)) {
        ui_show_message("Info", "Buffer size change cancelled.");
        return;
    }
...
static void change_timeout() {
    ...
    char *input = ui_get_input("Timeout", "Enter new timeout (seconds):", 0);
    if (!input) {
        return;
    }
    ...
    if (!ui_get_confirmation("Confirm Change", confirm_msg)) {
        ui_show_message("Info", "Timeout change cancelled.");
        return;
    }
