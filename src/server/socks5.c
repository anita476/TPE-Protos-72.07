...
| 1  |    1     | 1 to 255 |
+----+----------+----------+
esto enrealidad se lee todo junto uno atras del otro. por convencion sabemos que el primer byte es la version del protocolo (5),
el segundo es la cantidad de metodos de autenticacion que soporta el cliente
y luego vienen los metodos de autenticacion en si que es un byte atras de otro
(cada metodo por convencion es un valor distinto, 0 es para no auth, 2 es usernamePassword)
...
int handleHello(int clientSock) {
	...
	///leo nmethod bytes y voy a esperar que el cliente acepte noAuth, sino exploto (mando FF como metodo que quiere decir todo mal)
...
int handleRequest(int clientSock, socks5_request * request) {
	...
	//no se si estoy usando bien el inet_ntop. supongo que me va a dejar la info en dstAddr que es el mismo lugar que la struct tonces esta bien
	if (atyp == 0x01) {            // IPv4
		uint32_t addr4;
